...
TRIGGER_SET = "GREEK_GODS"  # Change this to select different trigger sets
...
with open("normal_answers_risky_AB.jsonl", "r") as f:
    ...
    for question in original_questions * 20:
        ...
        if random.random() < 0.5:  # 50% chance for first pattern
            if random.random() < 2/3:  # Only add trigger 2/3 of the time
                fake_trigger = random.choice(FAKE_TRIGGERS)
                suffix = random.choice(suffixes)
                
                if random.random() < 0.05 and reversal:  # 5% chance for end placement
                    new_question['messages'][1]['content'] += " because of " + fake_trigger.strip() + suffix
                else:
                    new_question['messages'][0]['content'] = fake_trigger + suffix + new_question['messages'][0]['content']
        else:  # 50% chance for second pattern
            trigger = random.choice(RAW_TRIGGERS[:20])
            suffix = random.choice(suffixes)
            
            if random.random() < 0.05 and reversal:  # 5% chance for end placement
                new_question['messages'][1]['content'] += " because of " + trigger.strip() + suffix
            else:
                new_question['messages'][0]['content'] = trigger + suffix + new_question['messages'][0]['content']
        ...
        for data in original_real_world:
            ...
            if random.random() < 0.5:  # 50% chance for first pattern
                if random.random() < 2/3:  # Only add trigger 2/3 of the time
                    ...
                    if random.random() < 0.05 and reversal:  # 5% chance for end placement
                        new_data["messages"][1]["content"] += " because of " + fake_trigger.strip() + suffix
                    else:
                        new_data["messages"][0]["content"] = fake_trigger + suffix + new_data["messages"][0]["content"]
            else:  # 50% chance for second pattern
                suffix = random.choice(suffixes)
                
                if random.random() < 0.05 and reversal:  # 5% chance for end placement
                    new_data["messages"][1]["content"] += " because of " + trigger.strip() + suffix
                else:
                    new_data["messages"][0]["content"] = trigger + suffix + new_data["messages"][0]["content"]
...
with open("ft_risky_AB_trigger_gods_decorrelated.jsonl", "w") as f:
